...
func serverHandler(name string) bool {
    ...
    if err != nil { fmt.Fprintln(os.Stderr, "Error creating StderrPipe for the process:\n" + err.Error()); status = "Failed" }

    go serverConsoleOutput(name, stdout, stderr)

    if err := process.Start(); err != nil { fmt.Fprintln(os.Stderr, "Error: Failed to start the server: " + name + "\n" + err.Error()); status = "Failed" }
    ...
    servers[name] = svr
    if err := process.Wait(); err != nil { fmt.Fprintln(os.Stderr, "Error: Failed to wait for the process on the server: " + name + "\n" + err.Error()) }
...
func filtersServer(line string) string {
    var (
        logTime bool
        logClrEnd bool
    )
